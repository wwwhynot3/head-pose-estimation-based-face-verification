...
from algorithm.base import prcnn, mtcnn
import cv2
...
def detect_face(frame, min_prob=0, landmark=False):
    if landmark:
        boxes, probs, landmark = mtcnn.detect(frame, landmarks=True)
        boxes = np.array(boxes) if boxes is not None else np.array([])
        probs = np.array(probs) if probs is not None else np.array([])
        landmark = np.array(landmark) if landmark is not None else np.array([])
        valid_boxes, faces, probss = _crop_faces(frame, boxes, probs, min_prob)
        return valid_boxes, faces, probss, landmark
    else:

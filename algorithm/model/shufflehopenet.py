...
def createShuffledHopeNet(model_path, device='cpu'):
    shuffledhopenet = ShuffledHopeNet([4, 8, 4], [24, 116, 232, 464, 1024])
    shuffledhopenet.load_state_dict(torch.load(model_path, map_location=device), strict=False)
    shuffledhopenet.eval()
    return shuffledhopenet

def _shufflenetv2(*args, **kwargs):
    model = ShuffledHopeNet(*args, **kwargs)
